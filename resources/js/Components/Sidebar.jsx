...
const NAV_GROUPS = [
    ...
    },
    {
        ...
        items: [
            { key: 'agent', label: 'المساعد الذكي', routeName: 'agent.index', icon: 'robot' },
        ],
    },
];
...
const ICONS = {
    ...
    receipt: (p) => (
        <svg width="16" height="16" viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.5" {...p}>
            <path d="M5 2v16l2.5-1.5L10 18l2.5-1.5L15 18V2l-2.5 1.5L10 2 7.5 3.5 5 2z" strokeLinejoin="round" />
    ...
    card: (p) => (
        <svg width="16" height="16" viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.5" {...p}>
            ...
            <path d="M2 8h16M5 12.5h4" strokeLinecap="round" />
        </svg>
    ),
    target: (p) => (
        <svg width="16" height="16" viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.5" {...p}>
    ...
    tag: (p) => (
        <svg width="16" height="16" viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.5" {...p}>
            <path d="M17.5 10.5l-7 7L3 10V3h7l7.5 7.5z" strokeLinejoin="round" />
    ...
    robot: (p) => (
        <svg width="16" height="16" viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.5" {...p}>
            ...
            <path d="M10 3v3M8 2h4" strokeLinecap="round" />
            <circle cx="7.5" cy="10.5" r="1" fill="currentColor" stroke="none" />
            <circle cx="12.5" cy="10.5" r="1" fill="currentColor" stroke="none" />
            <path d="M7 13.5h6" strokeLinecap="round" />
            <path d="M3 11h1M16 11h1" strokeLinecap="round" />
        </svg>
    ),
    help: (p) => (
        <svg width="16" height="16" viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.5" {...p}>
            <circle cx="10" cy="10" r="7.5" />
            <path d="M8 8a2 2 0 1 1 2.8 1.8c-.6.3-.8.7-.8 1.4" strokeLinecap="round" />
    ...
    user: (p) => (
        <svg width="14" height="14" viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.5" {...p}>
            ...
            <path d="M3.5 17c.8-3.2 3.4-5 6.5-5s5.7 1.8 6.5 5" strokeLinecap="round" />
        </svg>
    ),
    logout: (p) => (
        <svg width="14" height="14" viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.5" {...p}>
            <path d="M12 3H4v14h8M8 10h9M14 6.5L17.5 10 14 13.5" strokeLinecap="round" strokeLinejoin="round" />
        </svg>
    ),
    coin: (p) => (
        <svg width="14" height="14" viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.5" {...p}>
            <circle cx="10" cy="10" r="7.5" />
            <path d="M10 5v10M7.5 7.5h4a1.5 1.5 0 0 1 0 3H8a1.5 1.5 0 0 0 0 3h4" strokeLinecap="round" />
        </svg>
    ),
    stack: (p) => (
        <svg width="14" height="14" viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.5" {...p}>
            <path d="M10 3l7 3.5L10 10 3 6.5 10 3zM3 10l7 3.5L17 10M3 13.5l7 3.5 7-3.5" strokeLinejoin="round" />
        </svg>
    ),
};
...
const formatMAD = (value) =>
    `${Number(value || 0).toLocaleString('ar-MA', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} MAD`;

const getInitials = (name) =>
    String(name).trim().split(/\s+/).slice(0, 2).map((n) => n[0]).join('').toUpperCase() || 'U';

/* ==================================================================
 * SAFE NUMBER COMPONENT
 * ================================================================== */
const SafeNum = memo(({ value, color = C.t1, size = 'text-[0.82rem]' }) => (
    <span
        dir="ltr"
...
SafeNum.displayName = 'SafeNum';

/* ==================================================================
 * BRAND MARK
 * ================================================================== */
const BrandMark = memo(({ collapsed }) => (
 ...
 * SIDEBAR ITEM
 * ================================================================== */
const SidebarItem = memo(({ item, count, collapsed, onNavigate, routeExists, routeCurrent }) => {
    const Icon = ICONS[item.icon] || ICONS.grid;
    const active = routeExists && routeCurrent;
    const disabled = item.soon || !routeExists;
    ...
    if (disabled) {
        return (
            ...
            >
                ...
                {!collapsed && (
                    <span className={`${F.ar} text-[0.78rem] flex-1 truncate`} style={{ color: C.t3 }}>
                        {item.label}
                    </span>
                )}
            </div>
    ...
    return (
        <Link
            ...
            title={collapsed ? item.label : undefined}
            className={[
                'relative flex h-10 items-center gap-3 border-r-2 transition-colors duration-150',
        ...
        >
            ...
            {!collapsed && count !== undefined && (
                <span
                    className={`${F.mono} text-[0.6rem] px-1.5 py-0.5 border`}
                    style={{
                        borderColor: active ? `${C.green}55` : C.b,
                        color: active ? C.green : C.t4,
                    }}
                >
                    {count}
                </span>
 ...
 * PROFILE SECTION
 * ================================================================== */
const ProfileSection = memo(({ collapsed, user, navSummary, onNavigated, profileExists, settingsExists }) => {
    ...
    const AvatarBadge = ({ size = 'h-8 w-8', textSize = 'text-[0.65rem]' }) =>
        avatarUrl ? (
            ...
            <span
                className={`${size} flex shrink-0 items-center justify-center border`}
    ...
    return (
        <div className="relative" ref={ref}>
            <button
                ...
                onClick={toggle}
                className={[
                    'flex w-full items-center gap-2.5 border p-2 transition-colors',
                    'hover:bg-white/[0.04] focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-inset',
                    collapsed ? 'justify-center' : '',
                ].join(' ')}
                style={{
                    borderColor: open ? `${C.green}55` : C.b,
                    background: open ? C.greenTrace : C.card2,
                    '--tw-ring-color': C.green,
                }}
            >
                <AvatarBadge />
            ...
            {open && (
                <div
                    className={[
                        'absolute z-[400] w-[280px] border shadow-2xl origin-bottom',
                        'tf-popup-animate',
                        collapsed ? 'bottom-0 right-full mr-2' : 'bottom-full mb-2 right-0',
                    ].join(' ')}
                        ...
                        <div className="border-b p-3" style={{ borderColor: C.b }}>
                            ...
                            <div className="grid grid-cols-2 gap-2">
                                <div className="border p-2" style={{ borderColor: C.b, background: C.card2 }}>
                                    ...
                                    <SafeNum value={formatMAD(navSummary.totalNetWorth)} color={C.green} size="text-[0.78rem]" />
                                </div>
                                <div className="border p-2" style={{ borderColor: C.b, background: C.card2 }}>
                                    <div className="flex items-center gap-1.5 mb-1">
                    ...
                    <div className="p-1.5 space-y-0.5">
                        ...
                        {settingsExists ? (
                            ...
                            >
                                ...
                                <span
                                    className={`${F.mono} text-[0.52rem] tracking-[1px] px-1.5 py-0.5 border`}
...
export default function Sidebar({ auth, mobileOpen = false, onMobileClose }) {
    ...
    // Cache route status - compute once, not on every render
    const routeStatusMap = useMemo(() => {
        ...
        NAV_GROUPS.forEach((group) => {
            group.items.forEach((item) => {
                if (!item.soon) {
                    map.set(item.key, {
                        exists: route().has(item.routeName),
                        current: route().has(item.routeName) && route().current(item.routeName),
                    });
                }
            });
        });
    ...
    const profileExists = useMemo(() => route().has('profile.edit'), []);
    const settingsExists = useMemo(() => route().has('settings.index'), []);
        ...
        <nav className="flex-1 overflow-y-auto px-2 pb-4">
            {NAV_GROUPS.map((group, gi) => (
                <div key={group.title} className={gi > 0 ? 'mt-6' : 'mt-5'}>
                    {!isCollapsed && (
                        <div
                            className={`${F.mono} text-[0.58rem] tracking-[2px] px-3 mb-1.5 flex items-center justify-between`}
                            style={{ color: C.t4 }}
                        >
                    ...
                    <div className="space-y-0.5">
                        {group.items.map((item) => {
                            const status = routeStatusMap.get(item.key);
                            return (
                                <SidebarItem
                                    key={item.key}
                                    item={item}
                                    ...
                                    routeExists={status?.exists ?? false}
                                    routeCurrent={status?.current ?? false}
                                />
                            );
                        })}
                    </div>
    ...
    return (
        <>
            <style>{`
                ...
                .tf-popup-animate { animation: tf-popIn 150ms ease-out; }
            `}</style>
            ...
            <aside
                dir="rtl"
                className={[
                    'fixed right-0 top-0 z-[200] hidden h-screen lg:flex flex-col border-l transition-all duration-300',
                    collapsed ? 'w-[68px]' : 'w-[240px]',
                ].join(' ')}
                style={{ background: C.card, borderColor: C.b }}
            >
                <div
                    className={`flex h-14 shrink-0 items-center border-b ${collapsed ? 'justify-center px-0' : 'justify-between px-4'}`}
                ...
                <div className="border-t p-2 space-y-1" style={{ borderColor: C.b }}>
                    <button
                        ...
                        className={[
                            'flex h-9 w-full items-center gap-3 transition-colors hover:bg-white/[0.04]',
                            'focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-inset',
            ...
            <aside
                dir="rtl"
                className={[
                    'fixed right-0 top-0 z-[250] flex h-screen w-[260px] flex-col border-l shadow-2xl',
