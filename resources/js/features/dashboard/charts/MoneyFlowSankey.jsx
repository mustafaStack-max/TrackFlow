...
export default function MoneyFlowSankey({ nodes = [], links = [], height = 470 }) {
    ...
    const run = async () => {
      ...
      chartRef.current.setOption({
        ...
        tooltip: {
          ...
          borderColor: C.bHot,
          padding: 10,
          textStyle: { color: C.t1, fontFamily: F.ar, fontSize: 12 },
          formatter: (p) => {
            if (p.dataType === 'edge') {
              return (
                `<b>${p.data.source}</b> ← <b>${p.data.target}</b><br/>` +
                `<span style="font-family:${F.mono};color:${C.green}">${fmtMAD(p.data.value)} MAD</span>`
              );
            }
            return (
              `<b>${p.name}</b><br/>` +
              `<span style="font-family:${F.mono};color:${C.green}">${fmtMAD(p.value)} MAD</span>`
            );
          },
        },
        series: [
          {
            type: 'sankey',
            /* ★ هوامش صغيرة جدًا ليتمدد الرسم داخل الحاوية أقصى تمدد */
            left: 8,
            right: 8,
            top: 12,
            bottom: 12,
  ...
  if (!links.length) {
    return <EmptyState>// لا توجد تدفقات في هذه الفترة //</EmptyState>;
  }
