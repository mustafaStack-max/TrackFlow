...
export default function ChangeAnalysisTable({ data = null }) {
  if (!data || !data.categories || data.categories.length === 0) {
    return <EmptyState>// لا توجد مصاريف لمقارنتها في هاتين الفترتين //</EmptyState>;
  }
  ...
  const totalChangeColor =
    direction === 'up' ? C.red : direction === 'down' ? C.green : C.t3;
  const totalChangeIcon =
    direction === 'up' ? '▲' : direction === 'down' ? '▼' : '—';
  const maxAbsDiff = Math.max(...categories.map((c) => Math.abs(c.diff)), 1);
  const showTotalPct =
    Math.abs(previousTotal) >= 100 && totalChangePct !== null;
    ...
    <div className="flex flex-col gap-5">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <div className="border p-3" style={{ borderColor: C.b, background: C.card2 }}>
          <div className={`${F.mono} text-[0.55rem] tracking-[2px] mb-1`} style={{ color: C.t4 }}>
            الفترة الحالية
          </div>
          <div className={`${F.mono} text-[1.1rem] font-bold`} style={{ color: C.t1 }}>
            {fmtMAD(currentTotal)}{' '}
            <span className="text-[0.6rem]" style={{ color: C.t4 }}>MAD</span>
          </div>
        </div>
        ...
        >
          ...
          <div className="flex items-baseline gap-2 flex-wrap">
            ...
            <span
              ...
              style={{
                borderColor: `${totalChangeColor}44`,
                color: totalChangeColor,
                background: `${totalChangeColor}15`,
              }}
            >
              {showTotalPct ? (
            ...
            >
              <div className="md:col-span-4 flex flex-col gap-1.5">
                <div className="flex items-center gap-2">
                  ...
                  {isNew && (
                    <span
                      className={`${F.mono} text-[0.5rem] tracking-[1px] px-1.5 py-0.5 border rounded`}
                      style={{ borderColor: C.amber, color: C.amber, background: `${C.amber}15` }}
                    >
                      جديد
                    </span>
                  )}
                  {isGone && (
                    <span
                      className={`${F.mono} text-[0.5rem] tracking-[1px] px-1.5 py-0.5 border rounded`}
                      style={{ borderColor: C.t4, color: C.t4 }}
                    >
                      اختفى
                    </span>
                  )}
